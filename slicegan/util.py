...
def test_img(pth, imtype, netG, nz = 64, lf = 4, periodic=False):
    ...
    if periodic:
        ...
        if periodic[2]:
            noise[:, :, :, :, :2] = noise[:, :, :, :, -2:]
    raw = netG(noise)
    microstructure_array = raw.detach().cpu().numpy()  # Assuming 'raw' is a PyTorch tensor, use .cpu().numpy() to convert to NumPy array

    # Save the microstructure as a NumPy array
    np.save('microstructure.npy', microstructure_array)
